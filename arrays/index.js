...
class Countries {
    ...
    sorted() {
        let _countries = JSON.parse(JSON.stringify(this.countries));
        _countries = _countries.forEach((v, i, a) => a[i][7] = v[7].sort((a, b) => a[2] < b[2] ? -1 : a[2] > b[2] ? 1 : 0));
        return new Countries(_countries);
    }
    ...
    generate() {
        ...
        countries.forEach((country) => {
            html += makeTableFromCountry(country);
        });
...
const cns = new Countries(countries);
const cnsSorted = cns.sorted();
document.getElementById('content').innerHTML = cnsSorted.generate();